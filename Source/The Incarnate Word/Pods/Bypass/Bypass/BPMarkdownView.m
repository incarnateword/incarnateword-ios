...
- (void)createAndDisplayViewsFromPageFrames:(CFArrayRef)pageFrames
    ...
    [self setContentSize:CGSizeMake(contentSize.width, contentSize.height + 120.0)];//ADITYA CHANGE
    //[self setContentSize:contentSize];//ADITYA ORIGINAL
    ...
    for (i = 0; i < count; i++) {
        ...
        BPMarkdownPageView *textView = [[BPMarkdownPageView alloc] initWithFrame:textViewFrame
                                                                       textFrame:textFrame];
        textView.backgroundColor = [UIColor clearColor];//ADITYA CHANGE, ORIGINAL no color was set
